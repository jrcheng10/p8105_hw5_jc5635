...
We first read and save raw data in a wide-format dataframe (i.e. as `df`), with associated file names and paths included.
...
We tidy `homicides` data by concatenating `city` and `state` into a `city_state` field, recoding incorrectly recorded values where relevant, creating a new `unsol_hm` field for unsolved homicides, and then summarizing counts of total and unsolved homicides by city.
...
We report the estimated proportion of unsolved homicides, and corresponding confidence intervals, for Baltimore, MD,  the `prop.test` function t
